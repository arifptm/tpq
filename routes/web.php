...
Route::get('/sis', function(){
    $u = App\Student::where('group_id',3)->where('stop_date',null)->where('institution_id','9')->orderBy('registered_date','asc')->get();
    foreach($u as $a){
	echo '"'.$a->fullname.'",';
	echo ($a->registered_date) ? Carbon\Carbon::parse($a->registered_date)->format("d-M-Y") : "";
	echo ";<br>";
    }
});