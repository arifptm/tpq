...
class StudentController extends Controller
{
    ...
    public function ajaxCreate(CreateStudent $request){        
        ...
        if ($request->image != ''){
            ...
            $request->image->move(public_path('/assets/students/'), $input['image']);
        }        
        $regdate = implode('-', array_reverse(explode("-", $request->registered_date)));
        $input['registered_date'] = $regdate;

        if($request->birth_date){
            $input['birth_date'] = implode('-', array_reverse(explode("-", $request->birth_date)));
        }

        $student = Student::create($input);
        
        $achievement['stage_id'] = 22; //new student as 'santri baru'
        $achievement['achievement_date'] = $regdate;
    ...
    public function ajaxUpdate(CreateStudent $request){        
        ...
        if($request->registered_date){
            $input['registered_date'] = implode('-', array_reverse(explode("-", $request->registered_date)));     
        }
        ...
        if($request->stop_date){
            $input['stop_date'] = implode('-', array_reverse(explode("-", $request->stop_date)));
        }
    ...
    public function studentAchievements($id){
        ...
        ($cnt > 0) ? $before = $achievements[0]->achievement_date : '';
        ...
        $datatable = Datatables::of($student)
            ...
            ->addColumn('name_href', function ($student) {
                ...
                if($student->group_id == 3){
                    $tpqd = "<i data-toggle='tooltip' title='Santri Dewasa' class='fa fa-user text-orange'></i>";
                }                
                return '<a href="/admin/students/'.$student->slug.'"><strong>'.str_limit($student->fullname,17,'~').'</strong></a>/'.$student->nickname.' '.$tpqd ;                    
            })      
            ...
            ->editColumn('image', function($student){
                $img = $student->image ?: 'default.jpg';
                return '<img src="/imagecache/thumbnail/'.$img.'">';
            })
            ...
            ->addColumn('actions', function($student) use($ui){
                ...
                $regdate = $student->registered_date ? \Carbon\Carbon::parse($student->registered_date)->format('d-m-Y') : '';
                $birthdate = $student->birth_date ? \Carbon\Carbon::parse($student->birth_date)->format('d-m-Y') : '';
                $stopdate = $student->stop_date ? \Carbon\Carbon::parse($student->stop_date)->format('d-m-Y') : '' ;
            ...
            ->rawColumns(['name_href','formated_registered_date','actions','status','gender_x','image']);
