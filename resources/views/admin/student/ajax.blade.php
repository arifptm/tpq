<script>  
  ...
  $(document).on('change.bs.fileinput', '.fileinput', function(){
    $('.fileinput-preview img').addClass('img-responsive img-circle')
  });

      $(document).on('hide.bs.modal', '#modal-edit-student', function (e) {      
    ...
    $('#status').on('ifChecked', function(event){      
      ...
      $('#stop_date').val(d+'-'+m+'-'+y).parent().slideDown();
    });
    ...
    $('#tpqd').on('ifChecked', function(event){      
      ...
      $('#phone').attr('placeholder', 'No. telepon');
      $('.parent_wrapper').slideUp();
    });

    $('#tpqd').on('ifUnchecked', function(event){
      $('#job').attr('placeholder', 'Pekerjaan orang tua');
      $('#phone').attr('placeholder', 'No. telepon orang tua');
    ...
    $('#btn-modal-create').click(function() {
      ...
      $('#jasny').removeClass('fileinput-exists').addClass('fileinput-new').fileinput('clear') 
      $("#institution_id").val($('#userInstitutionIds').text());

      var dt = new Date(); d = ('0'+(dt.getDate())).slice(-2); m = ('0'+(dt.getMonth()+1)).slice(-2); y = dt.getFullYear();      
      $("#registered_date").val(d+'-'+m+'-'+y);

      // $("#registration_number").val('02.20.02.04.');
    ...
    $('.modal-footer').on('click', '#submit-create', function(e) {      
      ...
      $.ajax({
        ...
        success: function(response){
          ...
          $('#myForm').find('input,textarea,select').not('[name=gender],[name=status],[name=group_id],[name=institution_id]',).val('');
    ...
    $(document).on('click', '#btn-modal-edit', function() {
      ...
      $("#infrastructure_fee").val($(this).data('infrastructure_fee'));
        ...
        if($(this).data('image')){
          $('#jasny').addClass('fileinput-exists').removeClass('fileinput-new')
          $('.fileinput-preview').html('<img src="/imagecache/medium_sq/' + $(this).data('image') + '" class="img-responsive img-circle" >');
        } else {
          $('#jasny').removeClass('fileinput-exists').addClass('fileinput-new')
        }
        // else {
        //   $("#dataimage").attr('src', '/imagecache/medium_sq/default.jpg');
        // }
      $('.modal-footer')
      .html("<button id='submit-update' class='btn btn-primary pull-left btn-lg'>Update</button><button class='btn bg-olive pull-left btn-lg' data-dismiss='modal'>Batal</button>")
